...
# Import new modules
from .dataset import DatasetDefinition, DatasetRegistry, FieldDefinition, create_sample_datasets
from .transformation import TransformationEngine, TransformationJob, TransformationStep, NaturalLanguageProcessor
from .config import DataFlowConfig, get_config
...
class DataFlow:
    ...
    def __init__(self, openai_api_key: Optional[str] = None, spark_config: Optional[Dict[str, str]] = None, config: Optional[DataFlowConfig] = None):
        """
        Initialize DataFlow with optional OpenAI API key and Spark configuration.
        
        Args:
            openai_api_key: OpenAI API key for pyspark-ai. If not provided, will try to get from config or environment.
            spark_config: Additional Spark configuration options.
            config: DataFlowConfig instance. If not provided, will use global config.
        """
        # Use provided config or get global config
        self.config = config or get_config()
        
        # Get OpenAI API key from parameter, config, or environment
        ...
        spark_builder = SparkSession.builder.appName(self.config.spark_app_name)
        
        # Get Spark configuration from config
        default_config = self.config.get_spark_config()
        
        # Add any additional spark_config
        if spark_config:
